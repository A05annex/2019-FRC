...
public class BumpTargetAngles extends Command {
     ...
     */
    public BumpTargetAngles(boolean bumpArm, boolean incDec) {
        super();
        this.bumpArm = bumpArm;
        this.inc = incDec ? Constants.BUMP_INC_ANGLE : -Constants.BUMP_INC_ANGLE;
    }
    ...
    protected void execute() {
        ...
        if (bumpArm) {
            if (xbox.getBumper(GenericHID.Hand.kLeft)) {
                Robot.armDriveTrain.bumpTargetAngles(0.0, inc, 0.0);
            } else {
                Robot.armDriveTrain.bumpTargetAngles(inc, 0.0, 0.0);

            }
        } else {
            Robot.armDriveTrain.bumpTargetAngles(0.0, 0.0, inc);
